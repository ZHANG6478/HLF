...
def _call_model(model_func, train_data, test_data, train_dates, test_dates):
    ...
    if isinstance(out, tuple):
        preds, y_true = out[0], out[1]
    else:
        preds, y_true = out, test_data
    return np.asarray(preds), np.asarray(y_true)
...
def main():
    ...
    for col in exchange_cols:
        ...
        for cfg in MODEL_CONFIG:
            ...
            try:
                ...
                n = min(len(preds), len(y_true), len(data["test_dates"]))
                preds = preds[:n]
                y_true = y_true[:n]
                ...
                plot_predictions(dates, y_true, preds, model_name=f"{name}-Ex{col}", title_suffix="Exchange")
                save_pred_table(dates, y_true, preds, model_name=f"{name}-Ex{col}", title_suffix="Exchange")
    ...
    for cfg in MODEL_CONFIG:
        ...
        rows = []
        for col in exchange_cols:
            if name in per_col_metrics[col].index:
                rows.append(per_col_metrics[col].loc[name].to_dict())
        avg_metrics[name] = pd.DataFrame(rows).mean().round(6).to_dict() if rows else dict(NAN_METRICS)

    average_metrics = pd.DataFrame(avg_metrics).T

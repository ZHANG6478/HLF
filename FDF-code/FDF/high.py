...
from predict import evaluate_high_modes_metrics
...
def run_one_category_high(
    category: str,
    data_file: str,
    high_model_name: str = "PatchTST",
    energy_thresh: float = 0.90,
    seq_len: int = 7,
    high_epochs: Optional[int] = None,
    refit_interval: Optional[int] = 0,
    k_min: int = 2,
    k_max: int = 10,
    alpha_min: int = 500,
    alpha_max: int = 1500,
    ...
    if not high_modes:
        print(f"[{category}] No high-frequency modes under threshold={energy_thresh}, skip.")
        return pd.DataFrame()

    df_high_metrics = evaluate_high_modes_metrics(
        modes=modes,
        all_dates=all_dates,
    ...
    if df_high_metrics.empty:
        return df_high_metrics

    df_high_info = df_sorted[df_sorted["Mode_ID"].isin(high_modes)][
        ["Mode_ID", "f_mean (Hz)", "Energy_Ratio (%)", "Cumulative_Energy"]
    ].copy()

    out = df_high_metrics.merge(df_high_info, on="Mode_ID", how="left")
    out.insert(0, "Category", category)
    out.insert(1, "Data_File", data_file)
    out.insert(2, "Energy_Thresh", energy_thresh)

    save_dir = os.path.join("result", "High_Model_Selection")
    os.makedirs(save_dir, exist_ok=True)
    out_path = os.path.join(save_dir, f"{category}_HighModes_{high_model_name}_Metrics.csv")
...
def main():
    ...
    save_dir = os.path.join("result", "High_Model_Selection")
    os.makedirs(save_dir, exist_ok=True)
    out_path = os.path.join(save_dir, f"ALL_Categories_HighModes_{high_model}_Metrics.csv")
